...
function MainPageD() {
  ...
  const insertStory = (e) => {
    ...
    return ( 
      ...
      <div class="user-wrapper">
        ...
        <div class="sidebar-menu">
            <ul>
                <li  className="active" onClick={()=>{setHomePage(!homePage)}} >
                     <span class="las la-home"></span>  home
                </li>
                <li onClick={()=>{setHomePage(!homePage)}}>
    ...
    <div className = "main-content" >
        ...
        <div>
        <h1> {
            pendingpost !== 0 ? pendingpost : <h6> There 's No Pending Fundraisers</h6> }</h1> 
            ...
            <button > See More < span class = "las la-arrow-right" >
            </span></button >
            </div>
             ...
             <tbody> {
                getfundraiser && getfundraiser.map((post, index) => {
                        ...
                        <tr key = { index } >
                            ...
                            <td> { post.title } </td> 
                            <td> {
                                post.is_deleted === 0 ? <span className = 'status yellow'
                                onClick = {
                                    () => { ChangeFundraiserType(post.id, 0) } } > </span> : post.is_deleted === 1 ? <span className='status green' onClick={()=>{ChangeFundraiserType(post.id ,1 )}}></span > : post.is_deleted === 2 ? <span className = 'status red'
                                onClick = {
                                    () => { ChangeFundraiserType(post.id, 2) } } > </span>  : console.log("UnKnown")}
